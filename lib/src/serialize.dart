...
void serializeStruct(Type self, SerialBuffer buffer, dynamic data, {SerializerState? state, allowExtensions = true}) {
  ...
  for (var field in self.fields) {
    if (dy.containsKey(field.name)) {
      ...
      field.type!.serialize?.call(field.type, buffer, dy[field.name], state: state, allowExtensions: allowExtensions && field == self.fields[self.fields.length - 1]);
    } else {
  ...
  try {
    ...
    for (var field in self.fields) {
      if (allowExtensions && field.type!.extensionOf != null && !buffer.haveReadData()) {
        ...
        result[field.name] = field.type!.deserialize?.call(field.type, buffer, state: state, allowExtensions: allowExtensions);
      }
    }
    return result;
  } catch (e) {
...
deserializeVariant(Type self, SerialBuffer buffer, {SerializerState? state, allowExtensions = true}) {
  ...
  return [field.name, field.type!.deserialize?.call(field.type, buffer, state: state, allowExtensions: allowExtensions)];
}
